...
import (
	...
	equinixmetal "kubeform.dev/provider-equinixmetal-api/api/provider"
...
const (
	KFCFinalizer       = "equinixmetal.kubeform.com/finalizer"
		...
		payLoad.Resources = []resourceStateV4{
			{
				...
				Name:           obj.GetName(),
				ProviderConfig: "provider[\"registry.terraform.io/equinix/metal\"]",
...
func updateStateField(rClient client.Client, ctx context.Context, intrfc map[string]interface{}, gv schema.GroupVersion, obj *unstructured.Unstructured, jsonit jsoniter.API) error {
	...
	if hasAnySensitiveField {
		...
		if secretRef != nil {
			...
			secretName = obj.GetName() + "-" + "equinixmetal" + "-" + "sensitive"
		}
...
func setProviderMeta(rClient client.Client, provider *tfschema.Provider, ctx context.Context, unstructuredObj *unstructured.Unstructured, server *tfschema.GRPCProviderServer, jsonit jsoniter.API) error {
	jsonit = GetJSONItr(equinixmetal.GetEncoder(), equinixmetal.GetDecoder())
	...
	providerSpec := &equinixmetal.EquinixmetalSpec{}
