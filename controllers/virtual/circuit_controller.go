...
// CircuitReconciler reconciles a Circuit object
...
// +kubebuilder:rbac:groups=virtual.equinixmetal.kubeform.com,resources=circuits,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=virtual.equinixmetal.kubeform.com,resources=circuits/status,verbs=get;update;patch

func (r *CircuitReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("circuit", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Circuit")
...
func (r *CircuitReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &virtualv1alpha1.Circuit{}); err != nil {
			klog.Error(err, "unable to set up auditor", virtualv1alpha1.Circuit{}.APIVersion, virtualv1alpha1.Circuit{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&virtualv1alpha1.Circuit{}).
