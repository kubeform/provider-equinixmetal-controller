...
// VlanAttachmentReconciler reconciles a VlanAttachment object
...
// +kubebuilder:rbac:groups=port.equinixmetal.kubeform.com,resources=vlanattachments,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=port.equinixmetal.kubeform.com,resources=vlanattachments/status,verbs=get;update;patch

func (r *VlanAttachmentReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("vlanattachment", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch VlanAttachment")
...
func (r *VlanAttachmentReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &portv1alpha1.VlanAttachment{}); err != nil {
			klog.Error(err, "unable to set up auditor", portv1alpha1.VlanAttachment{}.APIVersion, portv1alpha1.VlanAttachment{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&portv1alpha1.VlanAttachment{}).
