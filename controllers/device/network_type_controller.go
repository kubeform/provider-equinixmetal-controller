...
// NetworkTypeReconciler reconciles a NetworkType object
type NetworkTypeReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=device.equinixmetal.kubeform.com,resources=networktypes,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=device.equinixmetal.kubeform.com,resources=networktypes/status,verbs=get;update;patch

func (r *NetworkTypeReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("networktype", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch NetworkType")
...
func (r *NetworkTypeReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &devicev1alpha1.NetworkType{}); err != nil {
			klog.Error(err, "unable to set up auditor", devicev1alpha1.NetworkType{}.APIVersion, devicev1alpha1.NetworkType{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&devicev1alpha1.NetworkType{}).
