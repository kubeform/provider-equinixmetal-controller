...
// BgpSessionReconciler reconciles a BgpSession object
type BgpSessionReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=metal.equinixmetal.kubeform.com,resources=bgpsessions,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=metal.equinixmetal.kubeform.com,resources=bgpsessions/status,verbs=get;update;patch

func (r *BgpSessionReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("bgpsession", req.NamespacedName)
	...
	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch BgpSession")
...
func (r *BgpSessionReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &metalv1alpha1.BgpSession{}); err != nil {
			klog.Error(err, "unable to set up auditor", metalv1alpha1.BgpSession{}.APIVersion, metalv1alpha1.BgpSession{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&metalv1alpha1.BgpSession{}).
