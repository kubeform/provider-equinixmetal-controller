...
// IpAttachmentReconciler reconciles a IpAttachment object
type IpAttachmentReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=metal.equinixmetal.kubeform.com,resources=ipattachments,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=metal.equinixmetal.kubeform.com,resources=ipattachments/status,verbs=get;update;patch

func (r *IpAttachmentReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("ipattachment", req.NamespacedName)
	...
	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch IpAttachment")
...
func (r *IpAttachmentReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &metalv1alpha1.IpAttachment{}); err != nil {
			klog.Error(err, "unable to set up auditor", metalv1alpha1.IpAttachment{}.APIVersion, metalv1alpha1.IpAttachment{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&metalv1alpha1.IpAttachment{}).
