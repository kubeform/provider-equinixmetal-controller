...
// ReservedIPBlockReconciler reconciles a ReservedIPBlock object
type ReservedIPBlockReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=metal.equinixmetal.kubeform.com,resources=reservedipblocks,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=metal.equinixmetal.kubeform.com,resources=reservedipblocks/status,verbs=get;update;patch

func (r *ReservedIPBlockReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("reservedipblock", req.NamespacedName)
	...
	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ReservedIPBlock")
...
func (r *ReservedIPBlockReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &metalv1alpha1.ReservedIPBlock{}); err != nil {
			klog.Error(err, "unable to set up auditor", metalv1alpha1.ReservedIPBlock{}.APIVersion, metalv1alpha1.ReservedIPBlock{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&metalv1alpha1.ReservedIPBlock{}).
