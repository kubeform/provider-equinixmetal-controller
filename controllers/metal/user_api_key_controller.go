...
// UserAPIKeyReconciler reconciles a UserAPIKey object
type UserAPIKeyReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=metal.equinixmetal.kubeform.com,resources=userapikeys,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=metal.equinixmetal.kubeform.com,resources=userapikeys/status,verbs=get;update;patch

func (r *UserAPIKeyReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("userapikey", req.NamespacedName)
	...
	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch UserAPIKey")
...
func (r *UserAPIKeyReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &metalv1alpha1.UserAPIKey{}); err != nil {
			klog.Error(err, "unable to set up auditor", metalv1alpha1.UserAPIKey{}.APIVersion, metalv1alpha1.UserAPIKey{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&metalv1alpha1.UserAPIKey{}).
