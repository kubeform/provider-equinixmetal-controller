...
// ProjectSSHKeyReconciler reconciles a ProjectSSHKey object
type ProjectSSHKeyReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=metal.equinixmetal.kubeform.com,resources=projectsshkeys,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=metal.equinixmetal.kubeform.com,resources=projectsshkeys/status,verbs=get;update;patch

func (r *ProjectSSHKeyReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("projectsshkey", req.NamespacedName)
	...
	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ProjectSSHKey")
...
func (r *ProjectSSHKeyReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &metalv1alpha1.ProjectSSHKey{}); err != nil {
			klog.Error(err, "unable to set up auditor", metalv1alpha1.ProjectSSHKey{}.APIVersion, metalv1alpha1.ProjectSSHKey{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&metalv1alpha1.ProjectSSHKey{}).
