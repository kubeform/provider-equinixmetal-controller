...
// ProjectAPIKeyReconciler reconciles a ProjectAPIKey object
type ProjectAPIKeyReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=metal.equinixmetal.kubeform.com,resources=projectapikeys,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=metal.equinixmetal.kubeform.com,resources=projectapikeys/status,verbs=get;update;patch

func (r *ProjectAPIKeyReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("projectapikey", req.NamespacedName)
	...
	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ProjectAPIKey")
...
func (r *ProjectAPIKeyReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &metalv1alpha1.ProjectAPIKey{}); err != nil {
			klog.Error(err, "unable to set up auditor", metalv1alpha1.ProjectAPIKey{}.APIVersion, metalv1alpha1.ProjectAPIKey{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&metalv1alpha1.ProjectAPIKey{}).
